...
{
	if (argc > 1 && (string)argv[0] == "HUFF")	// checks to see if the first value in argv is HUFF because the first will always be HUFF.exe
	{
		Huffman* tree = new Huffman();			// if it is create an instance of the huffman class

		if ((string)argv[1] == "-t")			// if the 2nd value of argv is -t create the tree for given file and output
		{
			if (argc == 3) tree->MakeTreeBuilder((string)argv[2]);							// if there is only an input file
			else if (argc == 4) tree->MakeTreeBuilder((string)argv[2], (string)argv[3]);	// if there is both and input file and output file
			else cout << "Did not properly enter -t command! Use -help, -h, -? for help.";	// other wise print out error
		}
		else if ((string)argv[1] == "-e")		// if the 2nd value of argv is -e encode a file
		{
			if (argc == 3) tree->EncoderFile((string)argv[2]);								// if the count is 3 only an input file has been enter
			else if (argc == 4) tree->EncoderFile((string)argv[2], (string)argv[3]);		// if the count is 4 an input and a output file has been entered
			else cout << "Did not properly enter -e command! Use -help, -h, -? for help.";	// other wise print out error
		}
		else if ((string)argv[1] == "-et")		// if the 2nd value of argv is -e encode a file with a predetermined tree file
		{
			if (argc == 4) tree->EncoderFileWithTree((string)argv[2], (string)argv[3]);							// if the count is 4 only the input file and tree file have been entered
			else if (argc == 5) tree->EncoderFileWithTree((string)argv[2], (string)argv[3], (string)argv[4]);	// if the count is 5 the input, tree and output file have been entered
			else cout << "Did not properly enter -et command! Use -help, -h, -? for help."; // other wise print out error
		}
		else if ((string)argv[1] == "-d")		// if the 2nd value of argv is -d decode a file 
		{
			if (argc == 4) tree->DecoderFile((string)argv[2], (string)argv[3]);	// if the count is 5 the input and outputfile has been entered
			else cout << "Did not properly enter -d command! Use -help, -h, -? for help."; // other wise print out error
		}
		else if ((string)argv[1] == "-help" || (string)argv[1] == "-?" || (string)argv[1] == "-h")
		{
